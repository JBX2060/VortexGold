#!/usr/bin/env bash
# Copyright (c) 2016 The Test Coin Super Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   testcoinsupercore.org and testcoinsuper.org and compares them.
...
WORKINGDIR="/tmp/testcoinsuper_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://testcoinsupercore.org"
HOST2="https://testcoinsuper.org"
BASEDIR="/bin/"
VERSIONPREFIX="testcoinsuper-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'testcoinsuper-',
...
if [ $? -ne 0 ]; then
   echo "testcoinsuper.org failed to provide signature file, but testcoinsupercore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "testcoinsuper.org and testcoinsupercore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Test Coin Super Core binary release signing key installed?"
   fi
