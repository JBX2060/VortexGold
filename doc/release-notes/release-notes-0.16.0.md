Test Coin Super Core version 0.16.0 is now available from:

  <https://testcoinsupercore.org/bin/testcoinsuper-core-0.16.0/>
...
installer (on Windows) or just copy over `/Applications/Test Coin Super-Qt` (on Mac)
or `testcoinsuperd`/`testcoinsuper-qt` (on Linux).
...
Test Coin Super Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

Test Coin Super Core should also work on most other Unix-like systems but is not
...
Test Coin Super Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
testcoinsuper data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Test Coin Super Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µTCS" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- testcoinsuper-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- testcoinsuper-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest testcoinsuper.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.testcoinsuper.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for testcoinsuper-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to testcoinsuper-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µTCS to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_testcoinsuper_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` testcoinsuper_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.testcoinsuper doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for testcoinsuper-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in testcoinsuper-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop testcoinsuperd (OmeGak)
...
- #10529 `331352f` Improve testcoinsuperd systemd service file (Flowdalic)
