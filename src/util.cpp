...
const char * const TESTCOINSUPER_CONF_FILENAME = "testcoinsuper.conf";
const char * const TESTCOINSUPER_PID_FILENAME = "testcoinsuperd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "testcoinsuperd -foo=bar
...
#else
    const char* pszModule = "testcoinsuper";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Test Coin Super
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Test Coin Super
    // Mac: ~/Library/Application Support/Test Coin Super
    // Unix: ~/.testcoinsuper
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Test Coin Super";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Test Coin Super";
#else
    // Unix
    return pathRet / ".testcoinsuper";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", TESTCOINSUPER_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", TESTCOINSUPER_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Test Coin Super Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Test Coin Super Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Test Coin Super Core developers";
    }
