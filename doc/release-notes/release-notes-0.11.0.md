Test Coin Super Core version 0.11.0 is now available from:

  <https://testcoinsuper.org/bin/testcoinsuper-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer TCS/kB fee to be
...
For example, add the following to `testcoinsuper.conf`:
...
related to the blockchain in the testcoinsuper system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Test Coin Super Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Test Coin Super Core will 
...
For an example script see [testcoinsuper-submittx](https://github.com/laanwj/testcoinsuper-submittx).
...
- #5334 `cf87536` libtestcoinsuperconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new testcoinsuper-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only TestCoinSuperAmountField
- #5753 `0f44672` Add testcoinsuper logo to about screen
...
- #5809 `46bfbe7` Add testcoinsuper-cli man page
...
- #5849 `41259ca` contrib/init/testcoinsuperd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Test Coin Super-Qt.app to Test Coin Super-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for testcoinsuper-tx)
